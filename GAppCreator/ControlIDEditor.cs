...
{
    public partial class IDsEditor : Form
    {
        Project prj;
        bool forControlID;
        public IDsEditor(Project _prj,bool useControlIDs)
        {
            prj = _prj;
            InitializeComponent();
            forControlID = useControlIDs;
            if (forControlID)
            {
                prj.ControlsIDs.Sort();
                foreach (ControlID cid in prj.ControlsIDs)
                {
                    dg.Rows.Add(new object[3] { cid.Name, cid.ID.ToString(), cid.Description });
                }
                this.Text = "Control IDs editor";
            }
            else
            {
                prj.ObjectEventsIDs.Sort();
                foreach (ControlID cid in prj.ObjectEventsIDs)
                {
                    dg.Rows.Add(new object[3] { cid.Name, cid.ID.ToString(), cid.Description });
                }
                this.Text = "Object events IDs editor";
            }
        }

        private void OnOK(object sender, EventArgs e)
        {
            List<ControlID> l = new List<ControlID>();
            for(int tr=0;tr<dg.Rows.Count;tr++)
            {
                if (GetCell(tr, 0).Length == 0)
                    continue;
                int value = -1;
                if ((int.TryParse(GetCell(tr,1),out value)==false) || (value<0))
                {
                    MessageBox.Show("Invalid number: '" + GetCell(tr, 1) + "' at row: " + (tr + 1).ToString());
                    return;
                }
                ControlID cid = new ControlID();
                cid.Name = GetCell(tr, 0);
                cid.ID = value;
                cid.Description = GetCell(tr, 2).Trim();
                l.Add(cid);
            }
            if (forControlID)
                prj.ControlsIDs = l;
            else
                prj.ObjectEventsIDs = l;
        ...
        private int FindNextValue()
        {
            ...
            for (int tr=0;tr<dg.Rows.Count;tr++)
            {
                string sValue = GetCell(tr, 1);
                if (sValue.Length>0)
                {
                    int value = 0;
                    if (int.TryParse(sValue, out value) == true)
                    {
                        if (value > max)
                            max = value;
                    }
        ...
        private void OnChangeCellValue(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex==0)
            {
                if ((sValue.Length > 0) && (GetCell(e.RowIndex,1).Length==0))
                    dg.Rows[e.RowIndex].Cells[1].Value = FindNextValue().ToString();
                if (sValue.Length == 0)
        ...
        private void OnValidateCell(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            {
                int value = -1;
                if ((int.TryParse(e.FormattedValue.ToString(),out value)==false) || (value<0))
                {
